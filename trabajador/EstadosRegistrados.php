                ...
                <div class="col">
                  <table class="table table-hover">
                    <thead class="thead-dark text-center">
                      <tr>
                        <th colspan="3">ESTADOS REGISTRADOS</th>
                      </tr>
                      <tr>
                        <td class="font-weight-bold text-left">ESTADO</td>
                        <td class="font-weight-bold text-left">PAIS</td>
                      </tr>                     
                    </thead>
                    <tbody>
                      <?php
                          $consulta_estados="SELECT E.ID, E.ESTADO, E.PAIS AS ID_PAIS, P.PAIS FROM ESTADOS AS E INNER JOIN PAISES AS P ON P.ID = E.PAIS ORDER BY ESTADO";
                          $resultado_estados=$turi->seleccionar($consulta_estados);
                          foreach ($resultado_estados as $fila) {
                            echo "<tr><td>$fila->ESTADO</td>
                                  <td>$fila->PAIS</td>
                                  <td>
                                    <div class='btn-group btn-block' role='group'>
                                      <button type='button' class='btn btn-info btn-editar' data-toggle='modal' data-target='#editar-estado' data-id='".$fila->ID."' data-estado='".$fila->ESTADO."' data-pais='".$fila->$ID_PAIS."'><i class='fas fa-pencil-alt'></i></button>";
                                      if($sesionROL>2){ echo"
