...
<head>
    ...
    <style></style>
    <title>TuriViajes - Buscador</title>
</head>
    ...
    <div class="container-fluid text-white">
        ...
        <div class="row jumbotron jumbotron-fluid d-flex justify-content-center"
            ...
            <div class=" w-50 text-dark">
                <h1 class="display-2">BUSCADOR</h1>
                <form action="#buscador.php" method="POST">
                    <div class="row">
                        <div class="form-group mt-2 col-12 col-md-8">
                            <input type="text" name="Busqueda1" class="form-control" id="destino" placeholder="Escribe la ciudad destino">
                        </div>
                        <div class="form-group mt-2 col-12 col-md-4">
                            <button type="submit" class="btn btn-outline-dark"title="Busqueda1ws"><i class="fas fa-search fa-lg my-auto mr-2"></i>
                            </button>
                        </div>
                    </div>
                </form>
        ...
        <div class="container text-light">
            <div class="row d-flex justify-content-center border-top border-bottom my-3">
                <h1 class="display-2 font-weight-bold ">Lo que encontramos para ti</h1><br>
                <small class="lead">Disfruta</small>
            </div>

            <div class="row">
                <?php              
                    $Busquedas=$_POST['Busqueda1'];      
                    $SQL="SELECT tours.ID, tours.TOUR, tours.DESCRIPCION, tours.DURACION, tours.CAPACIDAD,tours.PRECIO, tours.FOTO, tour_incluye.INCLUYE, tour_incluye.SERVICIOS_OPCIONALES, tour_no_incluye.NO_INCLUYE, notas.NOTAS FROM tours INNER JOIN tour_incluye on tour_incluye.TOUR=tours.ID INNER JOIN tour_no_incluye on tour_no_incluye.TOUR=tours.ID INNER JOIN notas on notas.TOUR=tours.ID INNER JOIN destino_tour ON destino_tour.TOUR = tours.ID INNER JOIN ciudades ON ciudades.ID = destino_tour.DESTINO WHERE ciudades.CIUDAD='$Busquedas'";

                    $PAIS="SELECT * FROM PAISES WHERE PAIS='Mexico' ORDER BY PAIS";
                    $RESULTADOp=$turi->seleccionar($PAIS);

                    $AEROLINEAS="SELECT aerolineas.AEROLINEA FROM TOURS INNER JOIN AEROLINEA_TOUR on aerolinea_tour.TOUR=tours.ID INNER JOIN AEROLINEAS on aerolineas.ID=aerolinea_tour.AEROLINEA GROUP BY aerolineas.AEROLINEA";
                    ...
                    //$SQL="SELECT tours.TOUR, tours.DESCRIPCION, tours.DURACION, tours.CAPACIDAD, tours.PRECIO, tours.FOTO  FROM TOURS WHERE TOURS.TOUR='Ciudad de México Cultura y Tradiciones'";
                    $RESULTADO=$turi->seleccionar($SQL);
        ...
        <!--FOOTER-->
        <div class="mt-5 row bg-dark footer">
